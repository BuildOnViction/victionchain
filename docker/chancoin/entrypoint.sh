...
params=""
accountsCount=$(
  chancoin account list --datadir $DATA_DIR  --keystore $KEYSTORE_DIR \
  2> /dev/null \
  | wc -l
)
...
# data dir
if [[ ! -d $DATA_DIR/chancoin ]]; then
  echo "No blockchain data, creating genesis block."
  chancoin init $genesisPath --datadir $DATA_DIR 2> /dev/null
fi
...
if [[ $accountsCount -le 0 ]]; then
  ...
  if [[ ! -z $PRIVATE_KEY ]]; then
    ...
    chancoin  account import ./private_key \
      --datadir $DATA_DIR \
