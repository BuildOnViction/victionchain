...
func (self *worker) wait() {
	for {
		...
		for result := range self.recv {
			...
			self.currentMu.Lock()
			stat, err := self.chain.WriteBlockWithState(block, work.receipts, work.state, work.chancoinxState)
			self.currentMu.Unlock()
			if err != nil {
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	if self.config.Posv != nil {
		chancoinX := self.eth.GetChancoinX()
		chancoinxState, err = chancoinX.GetChancoinxState(parent)
		if err != nil {
...
func (self *worker) commitNewWork() {
	...
	if atomic.LoadInt32(&self.mining) == 1 {
		...
		if self.config.Posv != nil && header.Number.Uint64()%self.config.Posv.Epoch != 0 && self.chain.Config().IsTIPChancoinX(header.Number) {
			chancoinX := self.eth.GetChancoinX()
			if chancoinX != nil && header.Number.Uint64() > self.config.Posv.Epoch {
				...
				txMatches, matchingResults = chancoinX.ProcessOrderPending(self.coinbase, self.chain, orderPending, work.state, work.chancoinxState)
				log.Debug("transaction matches found", "txMatches", len(txMatches))
			}
			txMatchBatch := &chancoinx_state.TxMatchBatch{
				Data:      txMatches,
				Timestamp: time.Now().UnixNano(),
				TxHash:    common.Hash{},
			}
			txMatchBytes, err := chancoinx_state.EncodeTxMatchesBatch(*txMatchBatch)
			if err != nil {
				log.Error("Fail to marshal txMatch", "error", err)
				return
			}
			nonce := work.state.GetNonce(self.coinbase)
			tx := types.NewTransaction(nonce, common.HexToAddress(common.ChancoinXAddr), big.NewInt(0), txMatchGasLimit, big.NewInt(0), txMatchBytes)
			...
			if err != nil {
				...
				if chancoinX != nil && chancoinX.IsSDKNode() {
		...
		ChancoinxStateRoot := work.chancoinxState.IntermediateRoot()
		tx := types.NewTransaction(work.state.GetNonce(self.coinbase), common.HexToAddress(common.ChancoinXStateAddr), big.NewInt(0), txMatchGasLimit, big.NewInt(0), ChancoinxStateRoot.Bytes())
