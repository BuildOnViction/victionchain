...
func NewBatchDatabaseWithEncode(datadir string, cacheLimit int) *BatchDatabase {
	...
	batchDB := &BatchDatabase{
		...
		emptyKey:   chancoinx_state.EmptyKey(), // pre alloc for comparison
...
func (db *BatchDatabase) GetOrderByTxHash(txhash common.Hash) []*chancoinx_state.OrderItem {
	return []*chancoinx_state.OrderItem{}
}

func (db *BatchDatabase) GetListOrderByHashes(hashes []string) []*chancoinx_state.OrderItem {
	return []*chancoinx_state.OrderItem{}
}
