...
func (db *MongoDatabase) GetObject(hash common.Hash, val interface{}) (interface{}, error) {
	...
	if cached, ok := db.cacheItems.Get(cacheKey); ok {
		...
		switch val.(type) {
		case *chancoinx_state.OrderItem:
			var oi *chancoinx_state.OrderItem
...
func (db *MongoDatabase) PutObject(hash common.Hash, val interface{}) error {
	...
	case *chancoinx_state.OrderItem:
		...
		// Store the key
		o := val.(*chancoinx_state.OrderItem)
...
func (db *MongoDatabase) CommitOrder(o *chancoinx_state.OrderItem) error {
...
func (db *MongoDatabase) GetOrderByTxHash(txhash common.Hash) []*chancoinx_state.OrderItem {
	var result []*chancoinx_state.OrderItem
	sc := db.Session.Copy()
	defer sc.Close()
...
func (db *MongoDatabase) GetListOrderByHashes(hashes []string) []*chancoinx_state.OrderItem {
	var result []*chancoinx_state.OrderItem
	sc := db.Session.Copy()
	defer sc.Close()
	...
	if err := sc.DB(db.dbName).C(ordersCollection).Find(query).All(&result); err != nil && err != mgo.ErrNotFound {
		...
		return []*chancoinx_state.OrderItem{}
	}
	return result
}
