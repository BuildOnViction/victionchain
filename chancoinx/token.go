...
import (
	...
	"github.com/chancoin-core/chancoin-gold"
...
func (chancoinx *ChancoinX) GetTokenDecimal(chain consensus.ChainContext, statedb *state.StateDB, coinbase common.Address, tokenAddr common.Address) (*big.Int, error) {
	if tokenDecimal, ok := chancoinx.tokenDecimalCache.Get(tokenAddr); ok {
		return tokenDecimal.(*big.Int), nil
	}
	if tokenAddr.String() == common.ChancoinNativeAddress {
		chancoinx.tokenDecimalCache.Add(tokenAddr, common.BasePrice)
	...
	chancoinx.tokenDecimalCache.Add(tokenAddr, tokenDecimal)
