...
func (t *Trade) SetBSON(raw bson.Raw) error {
	...
	t.Amount = chancoinx_state.ToBigInt(decoded.Amount)
	t.PricePoint = chancoinx_state.ToBigInt(decoded.PricePoint)

	t.MakeFee = chancoinx_state.ToBigInt(decoded.MakeFee)
	t.TakeFee = chancoinx_state.ToBigInt(decoded.TakeFee)
