...
func SubRelayerFee(relayer common.Address, fee *big.Int, statedb *state.StateDB) error {
	...
	log.Debug("ApplyChancoinXMatchedTransaction settle balance: SubRelayerFee BEFORE", "relayer", relayer.String(), "balance", balance)
	if balance.Cmp(fee) < 0 {
		return errors.Errorf("relayer %s isn't enough chancoin fee", relayer.String())
	} else {
		...
		log.Debug("ApplyChancoinXMatchedTransaction settle balance: SubRelayerFee AFTER", "relayer", relayer.String(), "balance", balance)
		return nil
	}
}
...
func CheckRelayerFee(relayer common.Address, fee *big.Int, statedb *state.StateDB) error {
	...
	if new(big.Int).Sub(balance, fee).Cmp(new(big.Int).Mul(common.BasePrice, common.RelayerLockedFund)) < 0 {
		return errors.Errorf("relayer %s isn't enough chancoin fee : balance %d , fee : %d ", relayer.Hex(), balance.Uint64(), fee.Uint64())
	}
	return nil
}
func AddTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB) error {
	// CHANCOIN native
	if token.String() == common.ChancoinNativeAddress {
		balance := statedb.GetBalance(addr)
		log.Debug("ApplyChancoinXMatchedTransaction settle balance: ADD TOKEN CHANCOIN NATIVE BEFORE", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		balance = big.NewInt(0).Add(balance, value)
		statedb.AddBalance(addr, value)
		log.Debug("ApplyChancoinXMatchedTransaction settle balance: ADD CHANCOIN NATIVE BALANCE AFTER", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
	...
	if statedb.Exist(token) {
		...
		log.Debug("ApplyChancoinXMatchedTransaction settle balance: ADD TOKEN BALANCE BEFORE", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		balance = balance.Add(balance, value)
		statedb.SetState(token, locHash, common.BigToHash(balance))
		log.Debug("ApplyChancoinXMatchedTransaction settle balance: ADD TOKEN BALANCE AFTER", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		return nil
	} else {
...
func SubTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB) error {
	...
	if token.String() == common.ChancoinNativeAddress {
		balance := statedb.GetBalance(addr)
		log.Debug("ApplyChancoinXMatchedTransaction settle balance: SUB CHANCOIN NATIVE BALANCE BEFORE", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		...
		log.Debug("ApplyChancoinXMatchedTransaction settle balance: SUB CHANCOIN NATIVE BALANCE AFTER", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
	...
	if statedb.Exist(token) {
		...
		locHash := common.BigToHash(GetLocMappingAtKey(addr.Hash(), slot))
		balance := statedb.GetState(token, locHash).Big()
		log.Debug("ApplyChancoinXMatchedTransaction settle balance: SUB TOKEN BALANCE BEFORE", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		if balance.Cmp(value) < 0 {
			return errors.Errorf("value %s in token %s not enough , have : %s , want : %s  ", addr.String(), token.String(), balance, value)
		}
		balance = balance.Sub(balance, value)
		statedb.SetState(token, locHash, common.BigToHash(balance))
		log.Debug("ApplyChancoinXMatchedTransaction settle balance: SUB TOKEN BALANCE AFTER", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		return nil
	} else {
		return errors.Errorf("token %s isn't exist", token.String())
	}
...
func CheckSubTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB, mapBalances map[common.Address]map[common.Address]*big.Int) (*big.Int, error) {
	...
	if token.String() == common.ChancoinNativeAddress {
		...
		log.Debug("CheckSubTokenBalance settle balance: SUB CHANCOIN NATIVE BALANCE ", "token", token.String(), "address", addr.String(), "balance", balance, "value", value, "newBalance", newBalance)
...
func CheckAddTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB, mapBalances map[common.Address]map[common.Address]*big.Int) (*big.Int, error) {
	...
	if token.String() == common.ChancoinNativeAddress {
		...
		log.Debug("CheckAddTokenBalance settle balance: ADD CHANCOIN NATIVE BALANCE ", "token", token.String(), "address", addr.String(), "balance", balance, "value", value, "newBalance", newBalance)
		return newBalance, nil
	}
	// TRC tokens
	if statedb.Exist(token) {
...
func CheckSubRelayerFee(relayer common.Address, fee *big.Int, statedb *state.StateDB, mapBalances map[common.Address]*big.Int) (*big.Int, error) {
	...
	if balance.Cmp(fee) < 0 {
		return nil, errors.Errorf("relayer %s isn't enough chancoin fee", relayer.String())
	} else {
