...
func GetSettleBalance(quotePrice *big.Int, takerSide string, takerFeeRate *big.Int, baseToken, quoteToken common.Address, makerPrice *big.Int, makerFeeRate *big.Int, baseTokenDecimal *big.Int, quoteTokenDecimal *big.Int, quantityToTrade *big.Int) (*SettleBalance, error) {
	...
	if takerSide == Bid {
		...
		takerFee = new(big.Int).Div(takerFee, common.ChancoinXBaseFee)
		// charge on the token he/she has before the trade, in this case: baseToken
		makerFee := new(big.Int).Mul(quoteTokenQuantity, makerFeeRate)
		makerFee = new(big.Int).Div(makerFee, common.ChancoinXBaseFee)
		...
		if baseToken.String() != common.ChancoinNativeAddress && quotePrice != nil && quotePrice.Cmp(common.Big0) > 0 {
		...
		} else if baseToken.String() == common.ChancoinNativeAddress {
			exMakerReceivedFee := new(big.Int).Mul(quantityToTrade, makerFeeRate)
			exMakerReceivedFee = exMakerReceivedFee.Div(exMakerReceivedFee, common.ChancoinXBaseFee)
			...
			exTakerReceivedFee = exTakerReceivedFee.Div(exTakerReceivedFee, common.ChancoinXBaseFee)
