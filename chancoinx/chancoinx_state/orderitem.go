...
func VerifyBalance(statedb *state.StateDB, chancoinxStateDb *ChancoinXStateDB, order *types.OrderTransaction, baseDecimal, quoteDecimal *big.Int) error {
	var quotePrice *big.Int
	if order.QuoteToken().String() != common.ChancoinNativeAddress {
		quotePrice = chancoinxStateDb.GetPrice(GetOrderBookHash(order.QuoteToken(), common.HexToAddress(common.ChancoinNativeAddress)))
	}
