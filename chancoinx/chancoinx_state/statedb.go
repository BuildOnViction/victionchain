...
func New(root common.Hash, db Database) (*ChancoinXStateDB, error) {
	tr, err := db.OpenTrie(root)
	if err != nil {
		return nil, err
	}
	return &ChancoinXStateDB{
		db:                       db,
		trie:                     tr,
...
func (self *ChancoinXStateDB) setError(err error) {
	if self.dbErr == nil {
		self.dbErr = err
	}
}

func (self *ChancoinXStateDB) Error() error {
	return self.dbErr
}

// Exist reports whether the given orderId address exists in the state.
// Notably this also returns true for suicided exchanges.
func (self *ChancoinXStateDB) Exist(addr common.Hash) bool {
	return self.getStateExchangeObject(addr) != nil
}

// Empty returns whether the state object is either non-existent
// or empty according to the EIP161 specification (balance = nonce = code = 0)
func (self *ChancoinXStateDB) Empty(addr common.Hash) bool {
	so := self.getStateExchangeObject(addr)
	return so == nil || so.empty()
}

func (self *ChancoinXStateDB) GetNonce(addr common.Hash) uint64 {
...
func (self *ChancoinXStateDB) GetPrice(addr common.Hash) *big.Int {
	stateObject := self.getStateExchangeObject(addr)
	if stateObject != nil {
...
func (self *ChancoinXStateDB) Database() Database {
	return self.db
}

func (self *ChancoinXStateDB) SetNonce(addr common.Hash, nonce uint64) {
...
func (self *ChancoinXStateDB) SetPrice(addr common.Hash, price *big.Int) {
	stateObject := self.GetOrNewStateExchangeObject(addr)
	if stateObject != nil {
...
func (self *ChancoinXStateDB) InsertOrderItem(orderBook common.Hash, orderId common.Hash, order OrderItem) {
...
func (self *ChancoinXStateDB) GetOrder(orderBook common.Hash, orderId common.Hash) OrderItem {
...
func (self *ChancoinXStateDB) SubAmountOrderItem(orderBook common.Hash, orderId common.Hash, price *big.Int, amount *big.Int, side string) error {
...
func (self *ChancoinXStateDB) CancelOrder(orderBook common.Hash, order *OrderItem) error {
...
func (self *ChancoinXStateDB) GetVolume(orderBook common.Hash, price *big.Int, orderType string) *big.Int {
	stateObject := self.GetOrNewStateExchangeObject(orderBook)
...
func (self *ChancoinXStateDB) GetBestAskPrice(orderBook common.Hash) (*big.Int, *big.Int) {
...
func (self *ChancoinXStateDB) GetBestBidPrice(orderBook common.Hash) (*big.Int, *big.Int) {
	stateObject := self.getStateExchangeObject(orderBook)
	if stateObject != nil {
...
func (self *ChancoinXStateDB) GetBestOrderIdAndAmount(orderBook common.Hash, price *big.Int, side string) (common.Hash, *big.Int, error) {
	stateObject := self.GetOrNewStateExchangeObject(orderBook)
	if stateObject != nil {
		var stateOrderList *stateOrderList
		switch side {
...
func (self *ChancoinXStateDB) updateStateExchangeObject(stateObject *stateExchanges) {
...
func (self *ChancoinXStateDB) getStateExchangeObject(addr common.Hash) (stateObject *stateExchanges) {
	// Prefer 'live' objects.
...
func (self *ChancoinXStateDB) setStateExchangeObject(object *stateExchanges) {
	self.stateExhangeObjects[object.Hash()] = object
	self.stateExhangeObjectsDirty[object.Hash()] = struct{}{}
}

// Retrieve a state object or create a new state object if nil.
func (self *ChancoinXStateDB) GetOrNewStateExchangeObject(addr common.Hash) *stateExchanges {
...
func (self *ChancoinXStateDB) MarkStateExchangeObjectDirty(addr common.Hash) {
...
func (self *ChancoinXStateDB) createExchangeObject(hash common.Hash) (newobj *stateExchanges) {
...
func (self *ChancoinXStateDB) Copy() *ChancoinXStateDB {
	self.lock.Lock()
	defer self.lock.Unlock()

	// Copy all the basic fields, initialize the memory ones
	state := &ChancoinXStateDB{
		db:                       self.db,
...
func (s *ChancoinXStateDB) clearJournalAndRefund() {
	s.journal = nil
	s.validRevisions = s.validRevisions[:0]
}

// Snapshot returns an identifier for the current revision of the state.
func (self *ChancoinXStateDB) Snapshot() int {
...
func (self *ChancoinXStateDB) RevertToSnapshot(revid int) {
...
func (s *ChancoinXStateDB) Finalise() {
...
func (s *ChancoinXStateDB) IntermediateRoot() common.Hash {
	s.Finalise()
	return s.trie.Hash()
}

// Commit writes the state to the underlying in-memory trie database.
func (s *ChancoinXStateDB) Commit() (root common.Hash, err error) {
	...
	log.Debug("ChancoinX Trie cache stats after commit", "misses", trie.CacheMisses(), "unloads", trie.CacheUnloads(), "root", root.Hex())
	return root, err
}
