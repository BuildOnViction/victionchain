...
func TestStableInts(t *testing.T) {
	data := ints
	sort.Stable(sort.IntSlice(data[0:]))
	if !sort.IntsAreSorted(data[0:]) {
		t.Errorf("nsorted %v\n   got %v", ints, data)
	}
}
...
func BenchmarkSort1e2(b *testing.B)   { bench(b, 1e2, sort.Sort, "Sort") }
func BenchmarkStable1e2(b *testing.B) { bench(b, 1e2, sort.Stable, "Stable") }
func BenchmarkSort1e4(b *testing.B)   { bench(b, 1e4, sort.Sort, "Sort") }
func BenchmarkStable1e4(b *testing.B) { bench(b, 1e4, sort.Stable, "Stable") }
func BenchmarkSort1e6(b *testing.B)   { bench(b, 1e6, sort.Sort, "Sort") }
func BenchmarkStable1e6(b *testing.B) { bench(b, 1e6, sort.Stable, "Stable") }
