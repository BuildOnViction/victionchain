...
func TestAddrAndBlockMap(t *testing.T) {
	...
	eq := reflect.DeepEqual(common.TomoAddrMap, addrMap) &&
		reflect.DeepEqual(common.TomoBlockMap, blockMap)

	if !eq {
		t.Errorf("TomoAddrMap or TomoBlockMap is not equal to the expected map.\nExpected:\n%v\nGot:\n%v", fmt.Sprintf("%#v", addrMap), fmt.Sprintf("%#v", common.TomoAddrMap))
		t.Errorf("TomoBlockMap is not equal to the expected map.\nExpected:\n%v\nGot:\n%v", fmt.Sprintf("%#v", blockMap), fmt.Sprintf("%#v", common.TomoBlockMap))
	}
	if len(common.TomoAddrMap) != len(addrMap) {
		t.Errorf("TomoAddrMap length mismatch: expected %d, got %d", len(addrMap), len(common.TomoAddrMap))
	}
	if len(common.TomoBlockMap) != len(blockMap) {
		t.Errorf("TomoBlockMap length mismatch: expected %d, got %d", len(blockMap), len(common.TomoBlockMap))
	}
}
