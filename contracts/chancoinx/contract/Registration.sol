...
contract AbstractCHANCOINXListing {
...
contract RelayerRegistration {
    ...
    address constant private chancoinNative = 0x0000000000000000000000000000000000000001;
    ...
    AbstractCHANCOINXListing private ChancoinXListing;
    ...
    constructor (address chancoinxListing, uint maxRelayers, uint maxTokenList, uint minDeposit) public {
        ChancoinXListing = AbstractCHANCOINXListing(chancoinxListing);
        RelayerCount = 0;
    ...
    function register(address coinbase, uint16 tradeFee, address[] memory fromTokens, address[] memory toTokens) public payable {
        ...
        // check valid tokens, token must pair with chancoin(x/CHANCOIN)
    ...
    function depositMore(address coinbase) public payable relayerOwnerOnly(coinbase) onlyActiveRelayer(coinbase) notForSale(coinbase) nonZeroValue {
        require(msg.value >= 1 ether, "At least 1 CHANCOIN is required for a deposit request");
    ...
    function indexOf(address[] memory chancoinPair, address target) internal pure returns (bool){
        for (uint i = 0; i < chancoinPair.length; i ++) {
            if (chancoinPair[i] == target) {
                return true;
            }
        }
        return false;
    ...
    function validateTokens(address[] memory fromTokens, address[] memory toTokens) internal returns(bool) {
        ...
        address[] memory chancoinPairs = new address[](fromTokens.length);
        address[] memory nonChancoinPairs = new address[](fromTokens.length);

        for (uint i = 0; i < toTokens.length; i++) {
            bool b = ChancoinXListing.getTokenStatus(toTokens[i]) || (toTokens[i] == chancoinNative);
            b = b && (ChancoinXListing.getTokenStatus(fromTokens[i]) || fromTokens[i] == chancoinNative);
            if (!b) {
                return false;
            }
            if (toTokens[i] == chancoinNative) {
                chancoinPairs[countPair] = fromTokens[i];
                countPair++;
            } else {
                if (fromTokens[i] == chancoinNative) {
                    chancoinPairs[countPair] = toTokens[i];
                    countPair++;
                }
                nonChancoinPairs[countNonPair] = toTokens[i];
                countNonPair++;
            }
        }

        for (uint j = 0; j < countNonPair; j++) {
            if (!indexOf(chancoinPairs, nonChancoinPairs[j])) {
                return false;
            }
        }
        return true;
    ...
    function addingTokenValidation(
        ...
        address[] memory chancoinPairs = new address[](RELAYER_LIST[coinbase]._toTokens.length + 1);

        if (fromToken == chancoinNative || toToken == chancoinNative) {
            return true;
        }

        bool b = ChancoinXListing.getTokenStatus(toToken) || (toToken == chancoinNative);
        b = b && (ChancoinXListing.getTokenStatus(fromToken) || fromToken == chancoinNative);
        if (!b) {
            return false;
        }

        // get tokens that paired with chancoin
        for (uint i = 0; i < RELAYER_LIST[coinbase]._toTokens.length; i++) {
            if (RELAYER_LIST[coinbase]._toTokens[i] == chancoinNative) {
                chancoinPairs[countPair] = RELAYER_LIST[coinbase]._fromTokens[i];
                countPair++;
            } else {
                if (RELAYER_LIST[coinbase]._fromTokens[i] == chancoinNative) {
                    chancoinPairs[countPair] = RELAYER_LIST[coinbase]._toTokens[i];
                    countPair++;
                }
            }
        }
        if (!indexOf(chancoinPairs, toToken)) {
            return false;
        }
        return true;
    }
