...
type blockChainTomox interface {
	...
	OrderStateAt(block *types.Block) (*trading_state.TradingStateDB, error)
...
type OrderPool struct {
	...
	currentOrderState *trading_state.TradingStateDB    // Current order state in the blockchain head
	pendingState      *trading_state.TomoXManagedState // Pending state tracking virtual nonces
...
func (pool *OrderPool) reset(oldHead, newblock *types.Block) {
	...
	pool.pendingState = trading_state.ManageState(orderstate)
...
func (pool *OrderPool) State() *trading_state.TomoXManagedState {
	pool.mu.RLock()
...
func (pool *OrderPool) validateOrder(tx *types.OrderTransaction) error {
	...
	if !tx.IsCancelledOrder() {
		...
		if err := trading_state.VerifyPair(cloneStateDb, tx.ExchangeAddress(), tx.BaseToken(), tx.QuoteToken()); err != nil {
			return err
		}
		...
		if orderType == OrderTypeLimit {
			...
			if err := trading_state.VerifyBalance(cloneStateDb, cloneTomoXStateDb, tx, baseDecimal, quoteDecimal); err != nil {
	...
	if !trading_state.IsValidRelayer(cloneStateDb, tx.ExchangeAddress()) {
