...
func (v *BlockValidator) ValidateMatchingOrder(statedb *state.StateDB, chancoinxStatedb *chancoinx_state.ChancoinXStateDB, txMatchBatch chancoinx_state.TxMatchBatch, coinbase common.Address) error {
	posvEngine, ok := v.bc.Engine().(*posv.Posv)
	if posvEngine == nil || !ok {
		return ErrNotPoSV
	}
	chancoinXService := posvEngine.GetChancoinXService()
	if chancoinXService == nil {
		return fmt.Errorf("chancoinx not found")
	}
	log.Debug("verify matching transaction found a TxMatches Batch", "numTxMatches", len(txMatchBatch.Data))
	matchingResult := map[common.Hash]chancoinx_state.MatchingResult{}
	for _, txMatch := range txMatchBatch.Data {
		...
		newTrades, newRejectedOrders, err := chancoinXService.ApplyOrder(coinbase, v.bc, statedb, chancoinxStatedb, chancoinx_state.GetOrderBookHash(order.BaseToken, order.QuoteToken), order)
		if err != nil {
			return err
		}
		matchingResult[order.Hash] = chancoinx_state.MatchingResult{
			Trades:  newTrades,
			Rejects: newRejectedOrders,
		}
	}
	if chancoinXService.IsSDKNode() {
...
func ExtractMatchingTransactions(transactions types.Transactions) ([]chancoinx_state.TxMatchBatch, error) {
	txMatchBatchData := []chancoinx_state.TxMatchBatch{}
	for _, tx := range transactions {
		if tx.IsMatchingTransaction() {
			txMatchBatch, err := chancoinx_state.DecodeTxMatchesBatch(tx.Data())
			if err != nil {
				return []chancoinx_state.TxMatchBatch{}, fmt.Errorf("transaction match is corrupted. Failed to decode txMatchBatch. Error: %s", err)
			}
