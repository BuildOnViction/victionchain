...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	for i, tx := range txs {
		...
		// Mirror the on-chain blacklist balance bypass
		bypassBlacklistBalance(statedb, block.Number(), msg.From())
...
// bypassBlacklistBalance mirrors the "Bypass blacklist address" workaround in
// core.ApplyTransaction. During real block processing that code restores the
// balance of a fixed set of accounts at specific historical blocks so their
// transactions can be executed. Tracing calls vm.ApplyMessage directly and
// therefore skips that path, causing "insufficient funds" errors when replaying
// the affected transactions. This helper re-applies the exact same balance
// injection before an off-chain ApplyMessage. It only touches the trace's
// in-memory state and never affects consensus.
func bypassBlacklistBalance(statedb *state.StateDB, blockNumber *big.Int, from common.Address) {
	...
	addrFrom := from.Hex()
	if addr, ok := blockMap[blockNumber.Int64()]; ok {
		if strings.ToLower(addr) == strings.ToLower(addrFrom) {
			...
			statedb.SetBalance(common.HexToAddress(addr), hBalance)
		}
	}
}
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	// Mirror the on-chain blacklist balance bypass.
	bypassBlacklistBalance(statedb, vmctx.BlockNumber, message.From())
