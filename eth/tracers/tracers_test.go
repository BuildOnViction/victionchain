...
func TestPrestateTracerCreate2(t *testing.T) {
	...
	*/
	...
	msg, err := tx.AsMessage(signer, nil, nil, false, params.MainnetChainConfig.IsExperimental(context.BlockNumber))
	if err != nil {
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			msg, err := tx.AsMessage(signer, nil, common.Big0, false, test.Genesis.Config.IsExperimental(context.BlockNumber))
			if err != nil {
				t.Fatalf("failed to prepare transaction for tracing: %v", err)
			}
			st := core.NewStateTransition(evm, msg, new(core.GasPool).AddGas(tx.Gas()))
