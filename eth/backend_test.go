package eth

import (
	"math/big"
	"testing"

	"github.com/tomochain/tomochain/params"
)

func TestRewardInflation(t *testing.T) {
	params.AllPosvProtocolChanges.TIPAdditionalBlockRewardBlock = big.NewInt(60)
	for i := int64(0); i < 200; i++ {
		// the first 2 years
		chainReward := currentChainReward(params.AllPosvProtocolChanges, big.NewInt(i))
		chainReward = rewardInflation(params.AllPosvProtocolChanges, chainReward, uint64(i), 10)

		// 3rd year, 4th year, 5th year
		halfReward := new(big.Int).Mul(new(big.Int).SetUint64(125), new(big.Int).SetUint64(params.Ether))
		if 20 <= i && i < 50 && chainReward.Cmp(halfReward) != 0 {
			t.Error("Fail to calculate reward inflation for 2 -> 5 years", "chainReward", chainReward)
		}

		// after 5 years and before TIPAdditionalBlockRewardBlock
		quarterReward := new(big.Int).Mul(new(big.Int).SetUint64(62.5*1000), new(big.Int).SetUint64(params.Finney))
		if 50 <= i && chainReward.Cmp(quarterReward) != 0 {
			t.Error("Fail to calculate reward inflation above 6 years", "chainReward", chainReward)
		}
	}
}
