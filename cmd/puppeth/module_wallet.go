...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'chancoin --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'chancoin --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ethstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh
