...
func TestDAOForkBlockNewChain(t *testing.T) {
	for i, arg := range []struct {
		...
		// {"", params.ChancoinMainnetChainConfig.DAOForkBlock, false},
...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	// Start a chancoin instance with the requested flags set and immediately terminate
	if genesis != "" {
		json := filepath.Join(datadir, "genesis.json")
		if err := ioutil.WriteFile(json, []byte(genesis), 0600); err != nil {
			t.Fatalf("test %d: failed to write genesis file: %v", test, err)
		}
		runChancoin(t, "--datadir", datadir, "init", json).WaitExit()
	} else {
		// Force chain initialization
		args := []string{"--port", "0", "--maxpeers", "0", "--nodiscover", "--nat", "none", "--ipcdisable", "--datadir", datadir}
		chancoin := runChancoin(t, append(args, []string{"--exec", "2+2", "console"}...)...)
		chancoin.WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "chancoin", "chaindata")
