...
func newLiquidationPriceState(db *TradingStateDB, orderBook common.Hash, price common.Hash, data itemList, onDirty func(price common.Hash)) *liquidationPriceState {
...
func (self *liquidationPriceState) createStateLiquidationPrice(db database.Database, lendingBook common.Hash) (newobj *stateLendingBook) {
	newobj = newStateLendingBook(self.db, self.orderBook, self.liquidationPrice, lendingBook, itemList{}, self.MarkStateLendingBookDirty)
...
func (self *liquidationPriceState) getTrie(db database.Database) database.Trie {
	if self.trie == nil {
		var err error
...
func (self *liquidationPriceState) updateTrie(db database.Database) database.Trie {
	tr := self.getTrie(db)
...
func (self *liquidationPriceState) updateRoot(db database.Database) error {
	self.updateTrie(db)
	if self.dbErr != nil {
		return self.dbErr
	}
...
func (self *liquidationPriceState) deepCopy(db *TradingStateDB, onDirty func(liquidationPrice common.Hash)) *liquidationPriceState {
	...
	for key, value := range self.stateLendingBooks {
		stateOrderList.stateLendingBooks[key] = value
	}
...
func (self *liquidationPriceState) getStateLendingBook(db database.Database, lendingBook common.Hash) (stateObject *stateLendingBook) {
	// Prefer 'live' objects.
...
func (self *liquidationPriceState) getAllLiquidationData(db database.Database) map[common.Hash][]common.Hash {
	...
	for _, lendingBook := range lendingBooks {
		...
		if stateLendingBook != nil {
			liquidationData[lendingBook] = stateLendingBook.getAllLendingIds(db)
		}
	}
	return liquidationData
}
