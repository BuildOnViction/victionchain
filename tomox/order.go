...
func (order *Order) UpdateQuantity(orderList *OrderList, newQuantity *big.Int, newTimestamp uint64, allowSave bool) error {
	if newQuantity.Cmp(order.Item.Quantity) > 0 && !bytes.Equal(orderList.Item.TailOrder, order.Key) {
		if err := orderList.MoveToTail(order, allowSave); err != nil {
			return err
		}
	}
